...
"""Ninja build configurator for input library"""
...
generator = generator.Generator(project = 'input', dependlibs = dependlibs, variables = [('bundleidentifier', 'com.maniccoder.input.$(binname)')])
...
if toolchain.is_monolithic() or target.is_ios() or target.is_android() or target.is_tizen():
  ...
  if target.is_ios():
    ...
    test_extrasources = [os.path.join('test', 'all', 'android', 'java', 'com', 'maniccoder', 'input', 'test', item) for item in [
      'TestActivity.java'
    ]]
