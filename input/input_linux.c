/* input_linux.c  -  Input library Linux implementation  -  Public Domain  -  2017 Mattias Jansson / Rampant Pixels
 *
...
static unsigned int lookup_key( KeySym sym )
{
	...
	switch( sym )
	{
		...
		case XK_KP_Enter: return KEY_RETURN;
		case XK_Up: return KEY_UP;
		case XK_Down: return KEY_DOWN;
		case XK_Left: return KEY_LEFT;
		case XK_Right: return KEY_RIGHT;
		case XK_BackSpace: return KEY_BACKSPACE;
		case XK_KP_Add: return KEY_NP_PLUS;
		case XK_KP_Subtract: return KEY_NP_MINUS;
		case XK_KP_Separator: return KEY_NP_DECIMAL;
		case XK_KP_Divide: return KEY_NP_DIVIDE;
		case XK_KP_Multiply: return KEY_NP_MULTIPLY;
		case XK_Num_Lock: return KEY_NP_NUMLOCK;
		...
		case 0xFE03: return KEY_RALT;
		case XK_Super_L: return KEY_LMETA;
		case XK_Super_R: return KEY_RMETA;
		...
		case XK_grave: return KEY_GRAVEACCENT;
...
void _input_service_parse_native( const void* opaque_event, const void* xic )
{
	...
	const XEvent* event = (const XEvent*)opaque_event;

	switch( event->type )
	{
		case MotionNotify:
		{
			const XPointerMovedEvent* moveevent = (const XPointerMovedEvent*)event;
			...
			input_event_post_mouse( INPUTEVENT_MOUSEMOVE, moveevent->x, moveevent->y, (real)( (int)moveevent->x - mouse_x ), (real)( (int)moveevent->y - mouse_y ), 0, 0, mouse_buttons );
		...
		case ButtonRelease:
		{
			...
			if( event->type == ButtonPress )
			{
				...
				input_event_post_mouse( INPUTEVENT_MOUSEDOWN, mouse_down_x[button], mouse_down_y[button], 0, 0, 0, button, mouse_buttons );
			}
			else
			{
				...
				mouse_buttons &= ~button;
				input_event_post_mouse( INPUTEVENT_MOUSEUP, moveevent->x, moveevent->y, dx, dy, time_elapsed( mouse_down_time[button] ), button, mouse_buttons );
			}
		...
		case MappingNotify:
		{
			const XMappingEvent* mapevent = (const XMappingEvent*)event;
			if( ( mapevent->request == MappingModifier ) || ( mapevent->request == MappingKeyboard ) )
			{
				XRefreshKeyboardMapping( (XMappingEvent*)mapevent );
			}
			break;
		}
		...
		case KeyPress:
		{
			const XKeyEvent* keyevent = (const XKeyEvent*)event;
			...
			if( event->type == KeyPress )
			{
				...
				{
					Status status;
					int num = Xutf8LookupString( (XIC)xic, (XKeyEvent*)keyevent, buf, bufsize, 0, &status );
					if( ( status == XLookupChars ) && ( num > 0 ) )
					{
						//Convert to UTF-16
						str = wstring_allocate_from_string( buf, num );
						/*log: return Stream: return get() << "Xutf8LookupString returned " << num << " UTF-8 characters:" << std: return endl << "  ";
						for( int i = 0; i < num; ++i )
							log: return Stream: return get() << "0x" << std: return hex << buf[i] << std: return dec << " (" << buf[i] << ")";
						log: return Stream: return get() << "" << std: return endl;
						log: return Stream: return get() << "Xutf8LookupString returned " << str.length() << " UTF-32 characters:" << std: return endl << "  ";
						for( unsigned int i = 0; i < str.length(); ++i )
							log: return Stream: return get() << "0x" << std: return hex << str.str()[i] << std: return dec << " (" << (char)str.str()[i] << ")";
						log: return Stream: return get() << "" << std: return endl;*/
					}
				}
				else
				{
					...
					int num = XLookupString( (XKeyEvent*)keyevent, buf, bufsize, 0, &compose );
					if( num > 0 )
						str = wstring_allocate_from_string( buf, num );
				}
				if( str )
				...
				{
					input_event_post_key( INPUTEVENT_CHAR, str[i], 0 );
				}
			}

			KeySym sym = XLookupKeysym( (XKeyEvent*)keyevent, 0 );
			
			if( event->type == KeyPress )
				input_event_post_key( INPUTEVENT_KEYDOWN, lookup_key( sym ), keyevent->keycode );
			else
				input_event_post_key( INPUTEVENT_KEYUP, lookup_key( sym ), keyevent->keycode );
			break;
		}
	}
}
