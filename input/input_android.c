/* input_android.h  -  Input library Android implementation  -  Public Domain  -  2017 Mattias Jansson / Rampant Pixels
 *
...
static uint16_t _keyevent_to_unicode( int64_t down_time, int64_t event_time, int32_t action, int32_t keycode, int32_t repeat, int32_t metastate, int32_t device_id, int32_t scancode, int32_t flags, int32_t source )
{
	thread_attach_jvm();
	...
	if( !method_getunicodechar )
	{
		...
		if( !class_keyevent ) { warn_logf( WARNING_SYSTEM_CALL_FAIL, "Unable to get android Java KeyEvent class" ); return 0; }
		method_ctor = (*jni_env)->GetMethodID( jni_env, class_keyevent, "<init>", "(JJIIIIIIII)V" );
		if( !method_ctor ) { debug_logf( "Unable to get Java KeyEvent ctor" ); return 0; }
		method_getunicodechar = (*jni_env)->GetMethodID( jni_env, class_keyevent, "getUnicodeChar", "()I" );
		if( !method_getunicodechar ) { debug_logf( "Unable to get Java KeyEvent getUnicodeChar method" ); return 0; }
	}

	jobject keyevent = (*jni_env)->NewObject( jni_env, class_keyevent, method_ctor, down_time, event_time, action, keycode, repeat, metastate, device_id, scancode, flags, source );
	if( !keyevent ) { debug_logf( "Unable to create Java KeyEvent object" ); return 0; }
...
int android_sensor_callback( int fd, int events, void* data )
{
	...
	for( int i = 0; i < num_events; ++i )
	{
		...
		//debug_logf( "Sensor vector:  %.3f  %.3f  %.3f", sensor_event->acceleration.x, sensor_event->acceleration.y, sensor_event->acceleration.z );
		input_event_post_acceleration( INPUTEVENT_ACCELERATION, sensor_event->acceleration.x, sensor_event->acceleration.y, sensor_event->acceleration.z );
	}
	return 1;
}
...
int32_t android_handle_input( struct android_app* app, AInputEvent* event )
{
	...
	if( AInputEvent_getType( event ) == AINPUT_EVENT_TYPE_MOTION )
	{
		...
		for( size_t pointer = 0; pointer < pointers; ++pointer )
		{
			...
			input_event_post_touch( id, x, y, dx, dy, ( id == INPUTEVENT_TOUCHEND ) ? t : velocity, finger, current_fingers );
	...
	else if( AInputEvent_getType( event ) == AINPUT_EVENT_TYPE_KEY )
	{
		...
		{
			input_event_post_key( ( action == AKEY_EVENT_ACTION_DOWN ) ? INPUTEVENT_KEYDOWN : INPUTEVENT_KEYUP, key_translator[keycode], keycode );

			if( action == AKEY_EVENT_ACTION_UP )
			{
				uint16_t unicode_char = _keyevent_to_unicode( down_time, event_time, action, keycode, repeat, metastate, device_id, scancode, flags, source );
				if( unicode_char )
				{
